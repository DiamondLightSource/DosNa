...
if __name__ == '__main__':
    ...
    for engine, backend in product(dn.engines.available, dn.backends.available):
        ...
        if not dn.compatible(engine, backend):
            log.warning('Skipping engine {} ({}) with {} backend'
                        .format(engine.name.capitalize(), engine.params, backend.name.capitalize()))
            continue
        ...
        with Timer('DosNa (engine: %s, backend: %s)' % (engine.name, backend.name)):
            ...
            with dn.Cluster('/tmp/') as C:
                ...
                with C.create_pool('test_dosna') as P:
                    data = P.create_dataset('test_data', data=rdata, chunks=(CS,CS,CS))
